...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate one saved prediction against stored payouts and results."
    )
...
def bootstrap_store(db_path: Path):
    cwd = Path.cwd()
    repo_candidates = [cwd, db_path.parents[2] if len(db_path.parents) >= 3 else cwd]
...
def main() -> None:
    ...
    result = store.evaluate_prediction_record(payload)
